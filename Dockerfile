# ===============================================================
# 🧠 Proyecto: Predicción de Ictus - Data Scientist / AI Developer
# Entorno: CPU-only (TensorFlow + scikit-learn + Streamlit + MLP Model)
# ===============================================================

FROM python:3.10-slim

# Evitar prompts interactivos y establecer variables de entorno
ENV DEBIAN_FRONTEND=noninteractive
ENV PYTHONUNBUFFERED=1
ENV PYTHONDONTWRITEBYTECODE=1
ENV PIP_DEFAULT_TIMEOUT=1000
ENV PIP_RETRIES=20

# Instalar dependencias del sistema necesarias para matplotlib y otras librerías
RUN apt-get update && apt-get install -y \
    build-essential \
    pkg-config \
    libhdf5-dev \
    libgl1-mesa-dri \
    libglib2.0-0 \
    && apt-get clean \
    && rm -rf /var/lib/apt/lists/*

# Crear y establecer el directorio de trabajo
WORKDIR /app

# Copiar requirements.txt e instalar dependencias de Python
COPY requirements.txt .
RUN pip install --no-cache-dir --retries ${PIP_RETRIES} -r requirements.txt

# Crear estructura de directorios
RUN mkdir -p /app/data /app/notebooks /app/src /app/app

# Copiar archivos del proyecto en orden específico para mejor caching
COPY data/ ./data/
COPY app/ ./app/
COPY src/ ./src/
COPY notebooks/ ./notebooks/

# Copiar archivos de configuración restantes
COPY . .

# Configurar matplotlib para entornos sin GUI
ENV MPLBACKEND=Agg

# Exponer puerto para Streamlit
EXPOSE 8501

# Variables de entorno para Streamlit
ENV LC_ALL=C.UTF-8
ENV LANG=C.UTF-8

# Comando para ejecutar la aplicación Streamlit desde el directorio correcto
CMD ["streamlit", "run", "app/app_web.py", "--server.address", "0.0.0.0", "--server.port", "8501"]
